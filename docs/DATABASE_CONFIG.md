# 🗄️ Конфигурация базы данных

## ⚠️ КРИТИЧЕСКИ ВАЖНО: Используемая база данных

**ИСПОЛЬЗУЕТСЯ ТОЛЬКО ОДНА БАЗА ДАННЫХ:**

- **Имя базы данных:** `b2bplatform`
- **Пользователь:** `postgres`
- **Пароль:** `Jnvnszoe5971312059001`
- **Хост:** `localhost`
- **Порт:** `5432`

**ПОЛНЫЙ DATABASE_URL:**
```
postgresql+asyncpg://postgres:Jnvnszoe5971312059001@localhost:5432/b2bplatform
```

## 🚨 ВАЖНО: Защита от потери данных

**НЕ МЕНЯТЬ базу данных без:**
1. Обновления всех документов
2. Обновления всех миграций
3. Обновления конфигурации
4. Резервного копирования текущей базы

**Перед изменением базы данных:**
1. Создайте резервную копию текущей базы
2. Обновите все документы
3. Обновите конфигурацию
4. Протестируйте подключение

## 📋 Где используется база данных

### Backend конфигурация
- **Файл:** `backend/app/config.py`
- **Переменная:** `DATABASE_URL`
- **Значение по умолчанию:** `postgresql+asyncpg://postgres:Jnvnszoe5971312059001@localhost:5432/b2bplatform`

### Переменные окружения
- **Файл:** `backend/.env`
- **Переменная:** `DATABASE_URL`
- **Должно быть:** `postgresql+asyncpg://postgres:Jnvnszoe5971312059001@localhost:5432/b2bplatform`

## 📊 Структура базы данных

### Основные таблицы проекта:
- `moderator_suppliers` - поставщики
- `keywords` - ключевые слова
- `supplier_keywords` - связь поставщиков и ключевых слов
- `blacklist` - черный список доменов
- `parsing_runs` - запуски парсинга
- `domains_queue` - очередь доменов
- `audit_log` - журнал аудита изменений

### Другие таблицы (могут быть из других проектов):
- `attachments` - вложения
- `otp_codes` - OTP коды
- `request_keys` - ключи запросов
- `request_recipients` - получатели запросов
- `requests` - запросы
- `users` - пользователи
- И другие...

## 🔧 Настройка

### 1. Обновление backend/.env

Создайте или обновите файл `backend/.env`:

```env
DATABASE_URL=postgresql+asyncpg://postgres:Jnvnszoe5971312059001@localhost:5432/b2bplatform
PARSER_SERVICE_URL=http://127.0.0.1:9003
ENV=development
LOG_LEVEL=INFO
LOG_SQL=false
CORS_ORIGINS=http://localhost:3000,http://127.0.0.1:3000
ATTACHMENTS_DIR=storage/attachments
```

### 2. Проверка подключения

```powershell
$env:PGPASSWORD="Jnvnszoe5971312059001"
psql -U postgres -d b2bplatform -h localhost -p 5432 -c "SELECT version();"
```

### 3. Применение миграций

```powershell
$env:PGPASSWORD="Jnvnszoe5971312059001"
psql -U postgres -d b2bplatform -h localhost -p 5432 -f backend\migrations\001_initial_schema.sql
psql -U postgres -d b2bplatform -h localhost -p 5432 -f backend\migrations\002_audit_log.sql
```

## 📝 Резервное копирование

### Создание бэкапа

```powershell
$env:PGPASSWORD="Jnvnszoe5971312059001"
pg_dump -U postgres -d b2bplatform -h localhost -p 5432 > backup_b2bplatform_$(Get-Date -Format "yyyyMMdd_HHmmss").sql
```

### Восстановление из бэкапа

```powershell
$env:PGPASSWORD="Jnvnszoe5971312059001"
psql -U postgres -d b2bplatform -h localhost -p 5432 < backup_b2bplatform_20251226_120000.sql
```

## ✅ Проверка целостности

Используйте скрипт проверки целостности:

```powershell
cd backend
python ../scripts/check_data_integrity.py
```

## 🔒 Безопасность

**⚠️ ВАЖНО:** Пароль базы данных хранится в открытом виде в конфигурации. Для production окружения:

1. Используйте переменные окружения
2. Не коммитьте `.env` файлы в Git
3. Используйте секреты/менеджеры паролей
4. Ограничьте доступ к базе данных

## 📚 Связанные документы

- `DATABASE_SETUP_GUIDE.md` - инструкция по настройке
- `APPLY_MIGRATIONS.md` - применение миграций
- `docs/TROUBLESHOOTING.md` - решение проблем
- `scripts/README.md` - скрипты для работы с БД













