import asyncio
import sys
from pathlib import Path

project_root = Path(__file__).parent.parent
ollama_dir = project_root / "ollama_inn_extractor"
if ollama_dir.exists() and str(ollama_dir) not in sys.path:
    sys.path.insert(0, str(ollama_dir))

from app.agents.browser_agent import BrowserAgent
from playwright.async_api import async_playwright

async def test_proactive():
    print("=" * 80)
    print("Quick test: Proactive search on mc.ru/company/contacts")
    print("=" * 80)
    
    try:
        playwright = await async_playwright().start()
        browser = await playwright.chromium.connect_over_cdp('http://127.0.0.1:9222')
        
        if browser.contexts:
            context = browser.contexts[0]
        else:
            context = await browser.new_context()
        
        if context.pages:
            page = context.pages[0]
        else:
            page = await context.new_page()
        
        # Navigate to contacts page
        print("\nNavigating to https://mc.ru/company/contacts...")
        agent = BrowserAgent()
        agent.page = page
        await agent.navigate('https://mc.ru/company/contacts')
        
        # Test proactive search
        print("\nTesting proactive search...")
        result = await agent.proactive_search_inn_on_page()
        
        print("\n" + "=" * 80)
        print("RESULTS")
        print("=" * 80)
        if result:
            print(f"[SUCCESS] Found INN: {result.get('inn')}")
            print(f"Source: {result.get('source')}")
            print(f"Has org form: {result.get('has_org_form')}")
            print(f"Context: {result.get('context', '')[:300]}")
        else:
            print("[FAILED] INN not found")
        
        await playwright.stop()
        
    except Exception as e:
        print(f"\n[ERROR] {e}")
        import traceback
        traceback.print_exc()

if __name__ == "__main__":
    asyncio.run(test_proactive())


